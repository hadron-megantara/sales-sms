        ...
        if($request->has('email') && $request->has('password')){
            ...
            if(!$user){
                return response()->json([
                    'success' => false,
                    'data' => null,
                    'error' => ['code' => null, 'message' => "User not found"],
                    'version' => env('API_VERSION', 'v1')
                ]);
            }
            ...
            if(!Hash::check($request->password, $user->password)){
                return response()->json([
                    'success' => false,
                    'data' => null,
                    'error' => ['code' => null, 'message' => "Wrong password"],
                    'version' => env('API_VERSION', 'v1')
                ]);
            }
            ...
            try {
                ...
                return response()->json([
                    'success' => false,
                    'data' => null,
                    'error' => ['code' => null, 'message' => "Service error, please try again."],
                    'version' => env('API_VERSION', 'v1')
                ]);
            }
        } else{
            return response()->json([
                'success' => false,
                'data' => null,
                'error' => ['code' => null, 'message' => "User or Password can not be empty"],
                'version' => env('API_VERSION', 'v1')
            ]);
        }
